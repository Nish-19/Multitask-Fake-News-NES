...
class Model:
    ...
    def evaluate(self, data):
        ...
        losses = []
        # save_logits = []
        # debug_file = open('pred_prob_details.txt', 'w')
        # print('Printing all predictions at once', file=debug_file)
        # logits_file = open('logits_debug.txt', 'w')
        for batch in tqdm(data[:self.args.eval_subset], desc='evaluating', leave=False):
            inputs, target = self.process_data(batch)
            with torch.no_grad():
                print(type(inputs))
                #pre_logits, output = self.network(inputs)
                output = self.network(inputs)
                loss = self.get_loss(output, target)
                ...
                np_logits = []
                # for ele in pre_logits.tolist():
                #     np_logits.append(numpy.array(ele))
                # save_logits.extend(np_logits)
                # print(pre_logits, pre_logits.size(), file=logits_file)
                # print(pred,file=debug_file)
        # print(predictions, file=debug_file)
        # predictions_df = pd.DataFrame(predictions)
        # predictions_df.to_csv('test_fnc_4ag_6dg.csv')
        # print('Length of predictions is', len(predictions), file=debug_file)
        # save_dict = {k:v for k, v in enumerate(save_logits)}
        # print(len(save_dict), file=logits_file)
        # with open('test_fnc_quora_emb_4ag_6dg.pickle', 'wb') as handle:
        ...
        # Here you have to return the predictions 0 or 1
