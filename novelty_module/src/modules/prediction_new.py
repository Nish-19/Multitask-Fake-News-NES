...
class AdvancedPrediction(Prediction):
    ...
    # Here have to selectively output the last second representations
    def forward(self, a, b):
        print('In Here - full')
        new_tensor = torch.cat([a, b, a - b, a * b], dim=-1)
        for i, l in enumerate(self.dense):
            ...
            if i == 1:
                pre_logits = x
        return pre_logits, self.dense(torch.cat([a, b, a - b, a * b], dim=-1))


@register('symmetric')
class SymmetricPrediction(AdvancedPrediction):
    def forward(self, a, b):
        print('In Here - symmetric')
        new_tensor = torch.cat([a, b, (a - b).abs(), a * b], dim=-1)
        for i, l in enumerate(self.dense):
            #print(l)
            if i==0:
                x = l(new_tensor)
            ...
            if i == 1:
                pre_logits = x
        return pre_logits, self.dense(torch.cat([a, b, (a - b).abs(), a * b], dim=-1))
